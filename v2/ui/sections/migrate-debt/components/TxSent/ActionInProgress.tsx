...
import { FlexDivCentered, ExternalLink } from '@snx-v1/styles';
...
const ActionInProgress: FC<ActionInProgressProps> = ({ amount, currencyKey, hash, action }) => {
  ...
  return (
    <Container>
      <SectionHeader>{t(`migrate-debt.actions.${action}.action.in-progress.title`)}</SectionHeader>
      <PendingConfirmation width="78" />
      <FlexDivCentered>
        <InfoContainer key="one">
          <InfoTitle>{t(`migrate-debt.actions.${action}.action.in-progress.depositing`)}</InfoTitle>
          <InfoData>{formatCurrency(currencyKey, amount, { currencyKey: currencyKey })}</InfoData>
        </InfoContainer>
      </FlexDivCentered>
      <SectionSubtext>
        {t(`migrate-debt.actions.${action}.action.in-progress.subtext`)}
      </SectionSubtext>
      {link ? (
        <StyledExternalLink href={link}>
          {t(`migrate-debt.actions.${action}.action.in-progress.etherscan`)}
        </StyledExternalLink>
      ) : null}
    </Container>
  );
