...
export const HistoricalTooltip = ({ payload, blockchains }: TvlProtocolsTooltipProps) => {
      ...
      {blockchains?.map((blockchain, index) => {
        return (
          <Flex key={index} mb={2} justifyContent="space-between" w="100%">
            <KeyColour label={tvlProtocols[blockchain].id} colour={BLOCKCHAIN_COLORS[index]} />
            <Text ml={3} fontFamily="heading" fontSize="12px" lineHeight="16px" textAlign="center">
              ${formatNumber(tvlProtocols[blockchain].cumDelegation)}
            </Text>
          </Flex>
        );
      })}
