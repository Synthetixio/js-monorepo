...
describe.only('generateGql', () => {
	...
	it('generates with basic options', () => {
		expect(
			generateGql(
				'foo',
				{ first: 50, orderBy: 'name', orderDirection: 'desc', where: { isOpen: true } },
				{ bar: true }
			)
	...
	it('generates with nested options', () => {
		expect(
			generateGql(
				'foo',
				{ first: 50, where: { name: 'boo' }, orderBy: 'name', orderDirection: 'desc' },
				{ bar: true }
			)
	...
	it('hex strings are quoted correctly', () => {
		expect(
			generateGql('foo', { where: { addr: '0x12341234123412341234' } }, { bar: true })
