...
export default function reactquery(schema: Schema): string {
	...
	for (const entity of schema.types) {
		...
		out.push(`export const useGet${entity.name}ById = <K extends keyof ${
			entity.name
		}Result>(url: string, options?: SingleQueryOptions, args?: ${
			entity.name
		}Args<K>, queryOptions: UseQueryOptions<Pick<${entity.name}Result, K>> = {}) => {
            ...
            return useQuery(
                ['codegen-graphql', enabled ? generateGql('${
									entity.name
								}', options, args), queryOptions?.queryKeys.toString() : null],
		...
		out.push(`export const useGet${entity.name}s = <K extends keyof ${
			entity.name
		}Result>(url: string, options?: MultiQueryOptions<${entity.name}Filter, ${
			entity.name
		}Result>, args?: ${entity.name}Args<K>, queryOptions: UseQueryOptions<Pick<${
			entity.name
		}Result, K>[]> = {}) => {
            ...
            return useQuery(
                ['codegen-graphql', enabled ? generateGql('${
									entity.name
								}s', options, args), queryOptions?.queryKeys.toString() : null],
                async () => func(url, options!, args!),
                {
                    ...queryOptions,
                    enabled: !!options && !!args,
