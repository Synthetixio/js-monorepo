...
export const requestHelper = async ({
	...
	if (prevOutput && prevOutput.length > 0) {
		...
		console.log('repeat newQuery', newQuery, 'responseVariables', {
			...responseVariables,
			autoGeneratedPaginationField,
		});
		/**
		 * NOTE there is an error sometimes on the last request so in this case I am
		 * returning hte results that have been returned so far
		 */
		let newResponse = {};
		try {
			newResponse = await request(endpoint, newQuery as RequestDocument, {
				...responseVariables,
				autoGeneratedPaginationField,
			});
		} catch (e) {
			console.log('e', e);
			return { [responseName]: prevOutput };
		}
...
// 	numTries?: BigNumber;
// }): Promise<any> => {
// 	console.log('prevOutput.length', prevOutput.length);
// 	const responseVariables = { ...variables };
// 	const resultsToDate = numTries.times(GQL_RESPONSE_LIMIT);
...
// 	if (prevOutput && prevOutput.length > 0) {
// 		const lastSortedField = prevOutput[0][sortedField];
// 		const numberRegex = /^\d+$/;
// 		const isSortedFieldNumber = numberRegex.test(lastSortedField);
// 		const newQuery = updateQueryString({
// 			query,
// 			sortedField,
// 			sortedFieldVariable,
// 			direction: query.indexOf('orderDirection: desc') !== -1 ? 'lt' : 'gt',
// 		});
// 		responseVariables[sortedFieldVariable] = isSortedFieldNumber
// 			? Number(lastSortedField)
// 			: lastSortedField;
...
// 				query,
// 				variables,
// 				prevOutput: [...sortBy(newResponseData, [sortedField]), ...prevOutput],
// 				numTries: numTries.plus(1),
// 			});
// 		}
...
// 				query,
// 				variables,
// 				prevOutput: sortBy(responseData, [sortedField]),
// 				numTries: numTries.plus(1),
// 			});
// 		}
// 		return response;
...
// 	direction: 'lt' | 'gt';
// }): string => {
// 	// if it is not already in the where clause add it
// 	if (query.indexOf(`${sortedField}_${direction}`) === -1) {
// 		const whereQueryStringArray = query.split('where: { ');
// 		whereQueryStringArray.splice(
// 			1,
// 			0,
// 			`where: { ${sortedField}_${direction}: $${sortedFieldVariable},`
// 		);
// 		return whereQueryStringArray.join('');
// 	}
// 	return query;
// };
