...
const useGetDelegateWallets = (ctx: QueryContext, authorizerAddress: string | null, options?: UseQueryOptions<[DelegationWallet]>) => {
	return useQuery<[DelegationWallet]>(
		...
		{
			enabled: ctx.networkId != null && !!authorizerAddress,
			...options,
