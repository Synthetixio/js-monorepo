...
export function convertType(t: Type): {
  name: string;
  arrayInfo?: { areItemsNullable: boolean };
  isNullable: boolean;
} {
  switch (t.kind) {
    case "NON_NULL":
    ...
    case "LIST":
      ...
      return {
        ...innerType,
        arrayInfo: { areItemsNullable: innerType.isNullable },
        isNullable: false,
      };
    default:
...
export function mapType(graphType: Type, destType: string) {
  ...
  switch (convertedType.name) {
    ...
    default:
      ...
      baseType =
        destType === "Result"
          ? `Partial<${convertedType.name}Result>`
          : `${convertedType.name}${destType}`;
  ...
  if (convertedType.arrayInfo) {
    tsTypeName = convertedType.arrayInfo.areItemsNullable
      ? `(${tsTypeName}|null)[]`
      : `${tsTypeName}[]`;
  }
