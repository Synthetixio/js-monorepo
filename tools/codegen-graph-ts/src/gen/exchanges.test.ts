...
describe('postgen exchanges', () => {
  ...
  it('pagination returns correct number of items', async () => {
    ...
    const res = await sgExchanges.getTotals(
      '',
      { first: 2400, orderBy: 'id', orderDirection: 'asc' },
      { id: true }
    );
    ...
    expect(mockPost).toHaveBeenCalledWith('', {
      query: '{totals(first:1000,orderBy:"id",orderDirection:"asc",where:{}){id}}',
    });
    expect(mockPost).toHaveBeenCalledWith('', {
      query: '{totals(first:1000,orderBy:"id",orderDirection:"asc",where:{id_gt:"hello999"}){id}}',
    });
  });
